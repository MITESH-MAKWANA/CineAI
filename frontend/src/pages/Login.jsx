...
export default function Login() {
  ...
  // If already signed in (e.g. came from onboarding), go straight to explore
  useEffect(() => {
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <div className="auth-header">
          ...
          <p className="auth-subtitle">Sign in to your CineAI account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="auth-form" id="login-form">
          ...
          <button type="submit" className="btn btn-primary btn-login" id="login-submit-btn" disabled={loading}>
            {loading ? <span className="btn-spinner" /> : <><span>Login</span><FiArrowRight size={17}/></>}
          </button>
        </form>
...
function AuthStyles() {
  return <style>{`
    ...
    .auth-forgot { text-align: right; margin-bottom: 0.75rem; }
    .auth-link-muted { color: var(--text-muted); font-size: 0.82rem; transition: var(--transition); }
    .auth-link-muted:hover { color: var(--accent-secondary); }
    .checkbox-group { display: flex; align-items: flex-start; gap: 10px; }
    .checkbox-group input[type=checkbox] { margin-top: 3px; accent-color: var(--accent-primary); }
    .checkbox-group label { font-size: 0.825rem; color: var(--text-secondary); line-height: 1.5; }
    .auth-divider { display: flex; align-items: center; gap: 12px; color: var(--text-muted); font-size: 0.78rem; margin: 0.25rem 0; }
