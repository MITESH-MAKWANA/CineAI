...
import { getWatchlist, removeFromWatchlist, getPosterUrl } from '../api/movies'
    ...
    <div className="list-page page-wrapper">
      <div className="container">
        <div className="list-page-header">
          <div className="list-page-icon" style={{ background: 'rgba(79,195,247,0.15)', color: 'var(--accent-blue)' }}>
            <FiBookmark size={28} fill="currentColor" />
          </div>
          <div>
            <h1 className="list-page-title">My Watchlist</h1>
            <p className="list-page-subtitle">{watchlist.length} movie{watchlist.length !== 1 ? 's' : ''} to watch</p>
          </div>
        </div>
              ...
              <div key={item.id} className="list-card card" onClick={() => navigate(`/movie/${item.movie_id}`)}>
                ...
                <div className="list-card-info">
                  ...
                  <div className="list-card-meta">
                    ...
                    <span className="list-date">Saved {item.added_at?.split('T')[0]}</span>
                  </div>
                  <button
      ...
      <style>{`
        ...
        .list-page-header { display: flex; align-items: center; gap: 1.25rem; padding: 2rem 0 1.75rem; border-bottom: 1px solid var(--border-subtle); margin-bottom: 2rem; }
        .list-page-icon { width: 64px; height: 64px; border-radius: var(--radius-lg); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        .list-empty { text-align: center; padding: 5rem 2rem; display: flex; flex-direction: column; align-items: center; gap: 1rem; }
        .list-empty h2 { font-size: 1.5rem; }
        .list-empty p { color: var(--text-muted); }
        .list-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 1.25rem; }
        ...
        .list-card-overlay { position: absolute; inset: 0; background: rgba(10,10,15,0.75); display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.3s; }
        ...
        .list-remove-btn { display: flex; align-items: center; gap: 5px; width: 100%; padding: 7px 10px; background: rgba(255,255,255,0.04); border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); color: var(--text-muted); font-size: 0.75rem; cursor: pointer; transition: var(--transition); }
        .list-remove-btn:hover { background: rgba(229,9,20,0.1); border-color: rgba(229,9,20,0.3); color: var(--accent-secondary); }
        @media (max-width: 480px) { .list-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; } }
      `}</style>
    </div>
  )
}
