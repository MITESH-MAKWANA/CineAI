...
from models.watchlist import FavoriteItem
...
def get_favorites(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    items = db.query(FavoriteItem).filter(FavoriteItem.user_id == current_user.id).all()
    return [
        {
...
def add_to_favorites(data: MovieItem, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    existing = db.query(FavoriteItem).filter(
        FavoriteItem.user_id == current_user.id,
        FavoriteItem.movie_id == data.movie_id
    ).first()
    if existing:
    ...
    db.add(item)
    db.commit()
    return {"message": "Added to favorites", "movie_id": data.movie_id}


@router.delete("/remove/{movie_id}")
def remove_from_favorites(movie_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    item = db.query(FavoriteItem).filter(
        FavoriteItem.user_id == current_user.id,
        FavoriteItem.movie_id == movie_id
    ).first()
    if not item:
...
def check_favorite(movie_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    exists = db.query(FavoriteItem).filter(
        FavoriteItem.user_id == current_user.id,
        FavoriteItem.movie_id == movie_id
    ).first()
    return {"in_favorites": bool(exists)}
